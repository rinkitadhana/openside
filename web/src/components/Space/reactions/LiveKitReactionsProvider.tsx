...
// Reactions are transient - only the overlay reads them - so keep a short
// tail instead of growing for the whole call.
const MAX_KEPT_REACTIONS = 200;

const appendReaction = (
  currentReactions: LiveKitReactionEvent[],
  reaction: LiveKitReactionEvent,
): LiveKitReactionEvent[] => {
  if (currentReactions.some((item) => item.id === reaction.id)) {
    return currentReactions;
  }
  const next = [...currentReactions, reaction];
  return next.length > MAX_KEPT_REACTIONS
    ? next.slice(next.length - MAX_KEPT_REACTIONS)
    : next;
};
...
export const LiveKitReactionsProvider = ({
  ...
  useEffect(() => {
    const handleTextStream = async (
      ...
      setReactions((currentReactions) =>
        appendReaction(currentReactions, nextReaction),
      );
    };
  ...
  const sendReaction = useCallback(
    async (emoji: string) => {
      ...
      setReactions((currentReactions) =>
        appendReaction(currentReactions, localReaction),
      );
    },
