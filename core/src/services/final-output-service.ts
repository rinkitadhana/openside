...
/** Parent finalOutputId for a rendition, or null if it doesn't exist. Used to
 *  authorize rendition mutations against the owning output's access rules. */
export async function getRenditionOutputId(
	renditionId: string,
): Promise<string | null> {
	const rendition = await prisma.finalOutputRendition.findUnique({
		where: { id: renditionId },
		select: { finalOutputId: true },
	});
	return rendition?.finalOutputId ?? null;
}
...
export async function canAccessFinalOutput(
	outputId: string,
	userId: string,
): Promise<boolean> {
	const output = await prisma.finalOutput.findUnique({
